    ...
    def split_grid_collate_fn(current_batch):
        ...
        def scrolling_window(image, mask, size, boundary=None):
            ...
            for x_stride in range(iterations):
                for y_stride in range(iterations):
                    ...
                    split_images.append(
                        image[:, x_bound_1:x_bound_2, y_bound_1:y_bound_2]
                    )
                    split_masks.append(mask[x_bound_1:x_bound_2, y_bound_1:y_bound_2])

                    if boundary is not None:
                        split_boundaries.append(
                            boundary[x_bound_1:x_bound_2, y_bound_1:y_bound_2]
                        )
        ...
        for batch in current_batch:
            ...
            if incl_boundary:
                ...
                split_sample, split_mask, split_boundary = scrolling_window(
                    sample, masks, original_size // factor, boundary=boundary
                )  # List of N elements of size C, 512, 512
                stacked_split_boundaries.append(torch.stack(split_boundary))
            else:
                split_sample, split_mask, _ = scrolling_window(
                    sample, masks, original_size // factor
                )  # List of N elements of size C, 512, 512

            stacked_split_samples.append(
                torch.stack(split_sample)
            )  # create tensor (N, C, 512, 512)
            stacked_split_masks.append(
                torch.stack(split_mask)
            )  # create tensor (N, 512, 512)

            all_filenames += name * len(
                split_sample
            )  # duplicate the file name. Could be used for matching purposes
...
def handle_concatenation(is_combined, is_split, image, pred, target, names):
    if is_combined:
        ...
        if "urban3d" in test_name or "spaceNet" in test_name:
            ...
            raise Exception(
                "something went wrong in handle_concat or data loading from combined"
            )
...
def handle_normal(image, pred, target, names):
    ...
    filename = names[0][
        random_index
    ]  # names is a list of tuples with the list len being 1 and the tuple len being the batch size
...
def handle_splits(split_factor, image, pred, target, names):
    ...
    split_image = (
        image[random_index_adj : random_index_adj + num_splits, :, :, :].cpu().numpy()
    )
...
def concatenate_images(split_tensor, split_factor):
    ...
    joined_image = np.block(
        [splits[i : i + split_factor] for i in range(0, num_splits, split_factor)]
    ).squeeze()
...
def save_and_test_images(sample, prefix):
    ...
    for i, split_img in enumerate(numpy_sample):
        ...
        matplotlib.image.imsave(
            "models/utils/testing/{}_{}.png".format(prefix, i), split_img
        )
